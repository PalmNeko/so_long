...
/*   Updated: 2024/06/21 18:55:46 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#include
int mxw_int_resize_hander(t_mxw_window *param)
{
	int					index;
	t_mxw_event_handler	*event;

	index = 0;
	while (index < param->event_handlers_cnt[ResizeRequest])
	{
		event = &param->event_handlers[ResizeRequest][index];
		event->handler(event->param);
		index++;
	}
	return (0);
